...
Ce graphique nous montre des statistiques de la saison **2022**, notament **le nombre de tirs, le nombre de buts et la probabilité
de marquer** selon les types de tirs:

![Buts par type de tirs](/public/goals_by_shot_type.png)

On remarque que le type de tirs le plus dangeureux est le **cradle** (**50%** de probabilité de but), suivit par le **bat**(**25.6%** de probabilité de but).
Le type de tir le plus courant est le **wrist**(il a été réalisé **48198** fois durant la saison 2022).

On a choisit ce graphique pour notre analyse car ses barres montrent bien de facon surperposée le nombre de tirs et
nombres de buts. De plus, il met bien en évidence la probabilité de buts à travers la courbe verte.

### Relation entre la distance et la probabilité de marquer un but

Pour mettre en évidence la relation entre la distance à la quelle un tir a été éffectué et la probabilité
...
On a choisit ce graphique pour notre analyse car avec ses courbes, on peut voir de facon intuitive l'évolution de
la variable sur l'axe y (la probabilité de marquer un buts) par rapport aux variations de la variable sur l'axe x (la distance).

### Pourcentage de but en fonction de la distance au filet et des types de tirs

Le graphique suivant montre l'évolution du pourcentage du buts en fonction de la distance du tir au but, ce pour différents types
...
De plus, pour la plus part des type de tirs, on observe la même tendance que précédement, c'est à dire
une diminution de la probabilité de buts quand la distance augmente. Le tir de type **wrap-around** fait exception car 
sa probabilité de but croit à partir de 45 pieds. Ca s'explique par le fait qu'a partir de 45 pieds, il n'y a que
...
- En dessous de 7 pieds et entre 30 et 44.9 pieds: c'est le tir de type **deflected** qui est le plus dangeureux
- Entre 7 et 29.9 pieds: c'est le tir de type **slap** qui est le plus dangeureux
- Au dessus de 45 pieds: c'est le tir de type **wrap-around** qui est le plus dangeureux
